<template>
  ...
  >
    <template v-slot:top>
      <v-toolbar flat>
        <v-toolbar-title>Akun Managements</v-toolbar-title>
...
export default {
  ...
  methods: {
    ...mapActions("akuntansiakun", ["index", "store", "edit", "update","delete"]),
    getDataFromApi() {
      this.loading = true;
    ...
    close() {
      this.dialog = false;
      this.$nextTick(() => {
        this.$store.commit("akuntansiakun/CLEAR_FORM");
        this.editedIndex = 0;
      });
    },
